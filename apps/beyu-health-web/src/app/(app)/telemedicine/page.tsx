...
async function fetchData(token: string) {
  try {
    ...
    const res = await fetch(`${base}/telemedicine/sessions?limit=20`, {
...
export default async function Page() {
  ...
  return (
    <div>
      <PageHeader title="Telemedicine" description="Virtual appointments, video consultations, secure messaging, file sharing, remote clinical documentation, consent." />
      <div className="panel">
        <div className="border-b border-border p-4 flex gap-3">
          <input className="field max-w-sm" placeholder="Search Telemedicine" />
          ...
          {data?.length || data?.items || Array.isArray(data) ? (
            <table className="w-full text-sm">
              <thead className="bg-surface-sunken text-xs uppercase tracking-widest text-ink-subtle">
                ...
                <th className="px-4 py-2 text-left">Practitioner</th>
                <th className="px-4 py-2 text-left">Status</th>
                <th className="px-4 py-2 text-left">Platform</th>
                <th className="px-4 py-2 text-left">Join URL</th>
              ...
              <tbody>
                {(Array.isArray(data) ? data : data.items ?? data.data ?? []).slice(0,20).map((p:any, idx:number) => (
                  ...
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.practitioner_name ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.status ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.platform ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.join_url ?? p.first_name ?? ""}</td>
            ...
            <div className="p-6 text-sm text-ink-muted">
              <div className="rounded-lg bg-navy-900 p-4 text-white text-xs mb-4">
                API: GET /api/v1/telemedicine/sessions — Real PostgreSQL, RLS tenant isolation, audit hash chain, RBAC/ABAC
