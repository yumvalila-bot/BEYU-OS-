...
async function fetchData(token: string) {
  try {
    ...
    const res = await fetch(`${base}/laboratory/orders?limit=20`, {
...
export default async function Page() {
  ...
  return (
    <div>
      <PageHeader title="Laboratory" description="Lab catalogue LOINC, test ordering, specimen collection/tracking, accessioning, results, critical values, verification, QC." />
      <div className="panel">
        <div className="border-b border-border p-4 flex gap-3">
          <input className="field max-w-sm" placeholder="Search Laboratory" />
          ...
          {data?.length || data?.items || Array.isArray(data) ? (
            <table className="w-full text-sm">
              ...
              <tbody>
                {(Array.isArray(data) ? data : data.items ?? data.data ?? []).slice(0,20).map((p:any, idx:number) => (
                  ...
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.order_number ?? p.first_name ?? ""}</td>
            ...
            <div className="p-6 text-sm text-ink-muted">
              <div className="rounded-lg bg-navy-900 p-4 text-white text-xs mb-4">
                API: GET /api/v1/laboratory/orders — Real PostgreSQL, RLS tenant isolation, audit hash chain, RBAC/ABAC
