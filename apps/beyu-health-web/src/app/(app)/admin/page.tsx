...
async function fetchData(token: string) {
  try {
    ...
    const res = await fetch(`${base}/tenants?limit=20`, {
...
export default async function Page() {
  ...
  const extra = token ? await (async () => {
    try {
      const base = process.env.NEXT_PUBLIC_HEALTH_API_URL ?? 'http://localhost:4001';
      const res = await fetch(`${base}/tenants/00000000-0000-0000-0000-000000000001/stats?limit=20`, { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' });
      if (!res.ok) return null;
  ...
  return (
    <div>
      <PageHeader title="Administration" description="Tenant management, facility management, departments, users, roles, permissions, services, pricing, billing, inventory, reporting." />
      <div className="panel">
        <div className="border-b border-border p-4 flex gap-3">
          <input className="field max-w-sm" placeholder="Search Administration" />
          ...
          {data?.length || data?.items || Array.isArray(data) ? (
            <table className="w-full text-sm">
              ...
              <tbody>
                {(Array.isArray(data) ? data : data.items ?? data.data ?? []).slice(0,20).map((p:any, idx:number) => (
                  ...
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.slug ?? p.first_name ?? ""}</td>
                  ...
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.country_code ?? p.first_name ?? ""}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="p-6 text-sm text-ink-muted">
              <div className="rounded-lg bg-navy-900 p-4 text-white text-xs mb-4">
                API: GET /api/v1/tenants — Real PostgreSQL, RLS tenant isolation, audit hash chain, RBAC/ABAC
                 | Extra: /tenants/00000000-0000-0000-0000-000000000001/stats
              </div>
