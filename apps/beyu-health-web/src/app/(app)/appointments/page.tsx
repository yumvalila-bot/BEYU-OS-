...
async function fetchData(token: string) {
  try {
    ...
    const res = await fetch(`${base}/appointments?limit=20`, {
...
export default async function Page() {
  ...
  return (
    <div>
      <PageHeader title="Appointments & Scheduling" description="Universal scheduling engine: appointments, walk-ins, recurring, provider/room/equipment schedules, queue management, reminders, waiting lists, telemedicine." />
      <div className="panel">
        <div className="border-b border-border p-4 flex gap-3">
          <input className="field max-w-sm" placeholder="Search Appointments & Scheduling" />
          ...
          {data?.length || data?.items || Array.isArray(data) ? (
            <table className="w-full text-sm">
              ...
              <tbody>
                {(Array.isArray(data) ? data : data.items ?? data.data ?? []).slice(0,20).map((p:any, idx:number) => (
                  ...
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.start ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.end ?? p.first_name ?? ""}</td>
            ...
            <div className="p-6 text-sm text-ink-muted">
              <div className="rounded-lg bg-navy-900 p-4 text-white text-xs mb-4">
                API: GET /api/v1/appointments — Real PostgreSQL, RLS tenant isolation, audit hash chain, RBAC/ABAC
