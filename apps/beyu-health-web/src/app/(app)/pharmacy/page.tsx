...
async function fetchData(token: string) {
  try {
    ...
    const res = await fetch(`${base}/pharmacy/prescriptions?limit=20`, {
...
export default async function Page() {
  ...
  const extra = token ? await (async () => {
    try {
      const base = process.env.NEXT_PUBLIC_HEALTH_API_URL ?? 'http://localhost:4001';
      const res = await fetch(`${base}/pharmacy/drugs?limit=20`, { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' });
      if (!res.ok) return null;
  ...
  return (
    <div>
      <PageHeader title="Pharmacy" description="Drug catalogue, formulary, prescriptions, dispensing, medication reconciliation, stock, batches, expiry, safety checks allergy/duplicate/interaction/dose." />
      <div className="panel">
        <div className="border-b border-border p-4 flex gap-3">
          <input className="field max-w-sm" placeholder="Search Pharmacy" />
          ...
          {data?.length || data?.items || Array.isArray(data) ? (
            <table className="w-full text-sm">
              ...
              <tbody>
                {(Array.isArray(data) ? data : data.items ?? data.data ?? []).slice(0,20).map((p:any, idx:number) => (
                  ...
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.drug_name ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.first_name ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.dosage_instruction ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.quantity ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.status ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.authored_on ?? p.first_name ?? ""}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="p-6 text-sm text-ink-muted">
              <div className="rounded-lg bg-navy-900 p-4 text-white text-xs mb-4">
                API: GET /api/v1/pharmacy/prescriptions — Real PostgreSQL, RLS tenant isolation, audit hash chain, RBAC/ABAC
                 | Extra: /pharmacy/drugs
              </div>
