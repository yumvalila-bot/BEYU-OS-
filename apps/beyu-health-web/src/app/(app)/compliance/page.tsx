...
async function fetchData(token: string) {
  try {
    ...
    const res = await fetch(`${base}/compliance/packs?limit=20`, {
...
export default async function Page() {
  ...
  return (
    <div>
      <PageHeader title="Compliance" description="Configurable compliance packs TZ MOH/MTUHA/NHIF/TMDA/TRA/Data Protection + ISO-27001, evidence, incidents, dashboard." />
      <div className="panel">
        <div className="border-b border-border p-4 flex gap-3">
          <input className="field max-w-sm" placeholder="Search Compliance" />
          ...
          {data?.length || data?.items || Array.isArray(data) ? (
            <table className="w-full text-sm">
              <thead className="bg-surface-sunken text-xs uppercase tracking-widest text-ink-subtle">
                ...
                <th className="px-4 py-2 text-left">Reqs</th>
              ...
              <tbody>
                {(Array.isArray(data) ? data : data.items ?? data.data ?? []).slice(0,20).map((p:any, idx:number) => (
                  ...
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.requirement_count ?? p.first_name ?? ""}</td>
            ...
            <div className="p-6 text-sm text-ink-muted">
              <div className="rounded-lg bg-navy-900 p-4 text-white text-xs mb-4">
                API: GET /api/v1/compliance/packs — Real PostgreSQL, RLS tenant isolation, audit hash chain, RBAC/ABAC
