...
async function fetchData(token: string) {
  try {
    ...
    const res = await fetch(`${base}/documents?limit=20`, {
...
export default async function Page() {
  ...
  return (
    <div>
      <PageHeader title="Documents" description="Clinical documents, scanned records, consent forms, invoices, prescriptions, referrals, reports, certificates, identity documents, contracts, images, PDFs." />
      <div className="panel">
        <div className="border-b border-border p-4 flex gap-3">
          <input className="field max-w-sm" placeholder="Search Documents" />
          ...
          {data?.length || data?.items || Array.isArray(data) ? (
            <table className="w-full text-sm">
              <thead className="bg-surface-sunken text-xs uppercase tracking-widest text-ink-subtle">
                ...
                <th className="px-4 py-2 text-left">Mime</th>
              ...
              <tbody>
                {(Array.isArray(data) ? data : data.items ?? data.data ?? []).slice(0,20).map((p:any, idx:number) => (
                  ...
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.document_type ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.mime_type ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 font-mono text-xs">{typeof p.size_bytes === "number" ? (p.size_bytes/100).toFixed(2) : (p.size_bytes ?? 0)}</td>
            ...
            <div className="p-6 text-sm text-ink-muted">
              <div className="rounded-lg bg-navy-900 p-4 text-white text-xs mb-4">
                API: GET /api/v1/documents — Real PostgreSQL, RLS tenant isolation, audit hash chain, RBAC/ABAC
