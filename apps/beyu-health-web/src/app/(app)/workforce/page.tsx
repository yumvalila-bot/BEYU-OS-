...
async function fetchData(token: string) {
  try {
    ...
    const res = await fetch(`${base}/workforce/practitioners?limit=20`, {
...
export default async function Page() {
  ...
  return (
    <div>
      <PageHeader title="Workforce" description="Employee/provider profiles, credentials, licenses, departments, roles, schedules, shifts, attendance, training expiry." />
      <div className="panel">
        <div className="border-b border-border p-4 flex gap-3">
          <input className="field max-w-sm" placeholder="Search Workforce" />
          ...
          {data?.length || data?.items || Array.isArray(data) ? (
            <table className="w-full text-sm">
              ...
              <tbody>
                {(Array.isArray(data) ? data : data.items ?? data.data ?? []).slice(0,20).map((p:any, idx:number) => (
                  ...
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.code ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.first_name ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.last_name ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.specialty ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.facility_name ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.employment_status ?? p.first_name ?? ""}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="p-6 text-sm text-ink-muted">
              <div className="rounded-lg bg-navy-900 p-4 text-white text-xs mb-4">
                API: GET /api/v1/workforce/practitioners — Real PostgreSQL, RLS tenant isolation, audit hash chain, RBAC/ABAC
