...
async function fetchData(token: string) {
  try {
    ...
    const res = await fetch(`${base}/billing/invoices?limit=20`, {
...
export default async function Page() {
  ...
  return (
    <div>
      <PageHeader title="Billing & Revenue Cycle" description="Service catalogue, pricing, invoices, payments, refunds, credit notes, discounts, deposits, receivables, revenue reporting." />
      <div className="panel">
        <div className="border-b border-border p-4 flex gap-3">
          <input className="field max-w-sm" placeholder="Search Billing & Revenue Cycle" />
          ...
          {data?.length || data?.items || Array.isArray(data) ? (
            <table className="w-full text-sm">
              ...
              <tbody>
                {(Array.isArray(data) ? data : data.items ?? data.data ?? []).slice(0,20).map((p:any, idx:number) => (
                  ...
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.invoice_number ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 truncate max-w-[200px]">{p.first_name ?? p.first_name ?? ""}</td>
                  <td className="px-4 py-2 font-mono text-xs">{typeof p.total_minor === "number" ? (p.total_minor/100).toFixed(2) : (p.total_minor ?? 0)}</td>
                  <td className="px-4 py-2 font-mono text-xs">{typeof p.paid_minor === "number" ? (p.paid_minor/100).toFixed(2) : (p.paid_minor ?? 0)}</td>
                  <td className="px-4 py-2 font-mono text-xs">{typeof p.balance_minor === "number" ? (p.balance_minor/100).toFixed(2) : (p.balance_minor ?? 0)}</td>
            ...
            <div className="p-6 text-sm text-ink-muted">
              <div className="rounded-lg bg-navy-900 p-4 text-white text-xs mb-4">
                API: GET /api/v1/billing/invoices — Real PostgreSQL, RLS tenant isolation, audit hash chain, RBAC/ABAC
