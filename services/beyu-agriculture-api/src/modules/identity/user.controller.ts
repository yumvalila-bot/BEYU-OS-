...
export class UserController {
  ...
  async list(@Query('q') q?: string, @Query('limit') limit?: string, @Query('offset') offset?: string) {
    ...
    if (ctx?.tenantId && !ctx.roles.includes('SUPER_ADMIN')) {
      ...
      where += ` AND EXISTS (SELECT 1 FROM agri_identity.memberships m WHERE m.user_id=u.id AND m.tenant_id=$${params.length})`;
    }
    if (q) {
      params.push(`%${q}%`);
    ...
    const rows = await this.db.query(
      `SELECT u.id, u.email, u.display_name, u.status, u.mfa_enabled, u.created_at FROM agri_identity.users u ${where} ORDER BY u.created_at DESC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`,
      [...params, lim, off],
    );
    const count = await this.db.query<{ count: string }>(`SELECT count(*)::text as count FROM agri_identity.users u ${where}`, params);
