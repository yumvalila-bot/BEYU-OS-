...
import { signJwt, verifyJwt, verifyPassword, generateToken } from '@beyu/security';
import { AGRICULTURE_OS_AUDIENCE } from '@beyu/agriculture-types';
import { UserRepository, type AgriUserRecord } from './user.repository';
...
export class AuthService {
  ...
  async resolveSecurityContext(token: string, meta: RequestMeta): Promise<AgriSecurityContext> {
    const config = loadConfig();
    const verification = verifyJwt(token, config.jwtSecret, AGRICULTURE_OS_AUDIENCE);
  ...
  private async issue(user: AgriUserRecord, meta: RequestMeta, mfaSatisfied: boolean, requestedTenantId: string | null, action: string): Promise<LoginResult> {
    const config = loadConfig();
